...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _initProfile() async {
    ...
    final savedUrl = prefs.getString('profile_pic_url');
    if (savedUrl != null && savedUrl.isNotEmpty) {
      _currentUser!.profileImageUrl = savedUrl;
    }
  ...
  Future<void> _onProfilePicTap() async {
    final should = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        content:
            const Text("Do you want to change your profile pic?"),
        actions: [
          TextButton(
    ...
    if (should == true) {
      ...
      if (picked != null) {
        ...
        await _uploadAndSaveProfilePic(file);
        // switch to network URL and refresh UI
        setState(() {
  ...
  Future<void> _uploadAndSaveProfilePic(File file) async {
    final key = AuthManager().currentUser!.email.replaceAll('.', '_');
    final ref = FirebaseStorage.instance
        .ref()
        .child('profile_pictures/$key.jpg');
    final upload = await ref.putFile(file);
    final url = await upload.ref.getDownloadURL();
    _currentUser!.profileImageUrl = url;
